...
class Trainer:
    ...
    def k_fold_iteration(self, num_epochs):
        ...
        for k in range(10):
            ...
            train_ls, valid_ls = self.train_k(valid, train, num_epochs, seed=k, fusion_method="kronecker",
                                              log_path='log/output.xlsx')
    ...
    def train_k(self, valid_dataset, train_dataset, train_num_epochs, seed, fusion_method, log_path):
        ...
        for epoch in range(train_num_epochs):
            with tqdm.tqdm(enumerate(train_dataloader, 0),
                ...
                for i, data in t:
                    ...
                    img1, img2, soft_bio1, soft_bio2, train_label = img1.to(self.device), img2.to(self.device), \
                                                                    soft_bio1.to(self.device), soft_bio2.to(
                        self.device), \
            ...
            log = {
                ...
                "train_loss": f'{train_ls[-1][0]}',
                "train_acc": f'{train_ls[-1][1]}',
                "valid_loss": "Null",
                "valid_acc": "Null"
            }
            df = pd.read_excel(log_path, index_col=0)
        ...
        with torch.no_grad():
            ...
            with tqdm.tqdm(enumerate(valid_dataloader),
                ...
                for j, data1 in t:
                    ...
                    img01, img02, soft_bio01, soft_bio02, valid_label = img01.to(self.device), img02.to(self.device), \
                                                                        soft_bio01.to(self.device), soft_bio02.to(
                        self.device), \
                                                                        valid_label.to(self.device)

                    output01, output02 = model(img01, img02, soft_bio01, soft_bio02)
            ...
            log = {
                ...
                "train_loss": "Null",
                "train_acc": "Null",
                "valid_loss": f'{valid_ls[-1][0]}',
                "valid_acc": f'{valid_ls[-1][1]}'
            }
            df = pd.read_excel(log_path, index_col=0)
            df = df.append([log], ignore_index=True)
            df.to_excel(log_path)
        torch.save(model.state_dict(), f'model_state_dict/with_fusion/100-k/model_{train_number_epochs}_{seed}.pkl')
...
def test_model(model, test_loader: DataLoader):
    model.eval()
    count = 0
    for img1, img2, soft_bio1, soft_bio2, lb in test_loader:
        img1, img2, soft_bio1, soft_bio2, lb = img1.to(t.device), img2.to(t.device), \
                                               soft_bio1.to(t.device), soft_bio2.to(t.device),\
                                               lb.to(t.device)
        out1, out2 = model(img1, img2, soft_bio1, soft_bio2)
